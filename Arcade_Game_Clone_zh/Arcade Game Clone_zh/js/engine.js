...
var doc = global.document,
    ...
    canvas.width = 496.3;//限制画布的宽
    canvas.height = 606;//限制画布的高
    ...
    var checkCollisions = function(){//如果玩家碰到了敌人回到原点函数
        for (var i =0; i < allEnemies.length; i++) {
            if (player.y == allEnemies[i].y && Math.abs(player.x - allEnemies[i].x) <= 50)  {
                player.x = 2 * width;
                player.y = 5 * height - delta;
            }
        }
        if (player.y === -delta)  {
            player.win();
    ...
    function render() {
        ...
        var rowImages = [
                'images/water-block.png',   // 这是第一行河
                'images/grass-block.png',   // 这是第一行草地
                'images/grass-block.png',   // 这是第二行草地
                'images/grass-block.png',   // 这是第三行草地
                'images/grass-block.png',   // 这是第四行草地
                'images/stone-block.png',   // 这是第一行石头
            ],
            numRows = 6,
            numCols = 5,
            row, col;
        ...
        for (row = 0; row < numRows; row++) {
            for (col = 0; col < numCols; col++) {
                 ...
                 */
                ctx.drawImage(Resources.get(rowImages[row]), col * 99, row * 80 );
            }
        }
